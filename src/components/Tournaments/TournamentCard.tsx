...
export const TournamentCard = ({ tournament, onJoin, onView, onPlay }: TournamentCardProps) => {
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Upcoming':
        return 'bg-yellow-100 dark:bg-yellow-600/20 text-yellow-700 dark:text-yellow-400 border-yellow-300 dark:border-yellow-600/40';
      case 'Active':
        return 'bg-green-100 dark:bg-green-600/20 text-green-700 dark:text-green-400 border-green-300 dark:border-green-600/40';
      case 'Completed':
        return 'bg-slate-100 dark:bg-slate-600/20 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-600/40';
      default:
        return 'bg-blue-100 dark:bg-blue-600/20 text-blue-700 dark:text-blue-400 border-blue-300 dark:border-blue-600/40';
    }
  };
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-slate-200 dark:border-blue-900/60 bg-white dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 hover:border-slate-300 dark:hover:border-blue-700/80 transition-all flex flex-col h-full shadow-sm">
      {/* Tournament Image Placeholder */}
      <div className="relative h-48 bg-gradient-to-br from-blue-100 to-purple-100 dark:from-blue-900/40 dark:to-purple-900/40 flex items-center justify-center">
        <div className="text-6xl text-slate-300 dark:text-white/20">🏆</div>
      ...
      <div className="bg-slate-50 dark:bg-slate-950 px-4 py-3 border-b border-slate-200 dark:border-blue-900/40">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-2">{tournament.description}</p>
      </div>
      ...
      <div className="p-4 space-y-3 flex-1 flex flex-col">
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <span className="text-blue-700 dark:text-blue-200">Start Date:</span>
            <p className="text-slate-900 dark:text-white font-semibold">{tournament.startDate}</p>
          </div>
          <div>
            <span className="text-blue-700 dark:text-blue-200">End Date:</span>
        ...
        <div className="flex justify-between items-center text-sm">
          <div>
            <span className="text-blue-700 dark:text-blue-200">Participants:</span>
          ...
          {tournament.prize && (
            <div>
              <span className="text-blue-700 dark:text-blue-200">Prize:</span>
              <span className="text-yellow-600 dark:text-yellow-400 font-semibold ml-2">{tournament.prize}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 pt-2 mt-auto">
          ...
          <Button
            className="flex-1 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-white text-sm rounded-lg font-semibold"
