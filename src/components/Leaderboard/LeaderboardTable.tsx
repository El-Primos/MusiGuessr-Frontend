...
const getRankColor = (rank: number, isCurrentUser: boolean): string => {
  ...
  switch (rank) {
    case 1:
      return 'text-yellow-600 dark:text-yellow-400 font-bold'; // Gold
    case 2:
      return 'text-slate-500 dark:text-slate-300 font-semibold'; // Silver
    case 3:
      return 'text-orange-600 dark:text-orange-400 font-semibold'; // Bronze
    default:
...
export const LeaderboardTable = ({ data, activeTab, isAuthenticated, currentUserId }: LeaderboardTableProps) => {
  return (
    <div className="rounded-lg overflow-hidden border border-slate-200 dark:border-blue-900/60 bg-white dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 shadow-sm">
      {/* Table Header */}
      <div className="bg-slate-50 dark:bg-slate-950 px-4 py-3 flex font-semibold text-sm border-b border-slate-200 dark:border-blue-900/40">
        <div className="w-16 text-blue-700 dark:text-blue-200">#</div>
        <div className="flex-1 text-blue-700 dark:text-blue-200">Player Name</div>
        <div className="w-32 text-right text-blue-700 dark:text-blue-200">Score</div>
      </div>
            ...
            return (
              <div
                ...
                className={`
                  ...
                  hover:bg-slate-50 dark:hover:bg-blue-900/20
                  transition-colors
              ...
              >
                ...
                {/* Player Name */}
                <div className={`flex-1 ${isCurrentUser ? 'text-blue-700 dark:text-blue-300 font-semibold' : 'text-blue-700 dark:text-blue-200'}`}>
                  {entry.playerName}
                </div>

                {/* Score */}
                <div className={`w-32 text-right font-semibold ${isCurrentUser ? 'text-blue-700 dark:text-blue-300' : 'text-slate-900 dark:text-white'}`}>
